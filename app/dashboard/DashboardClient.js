...
function DashboardContent({
  ...
  /** Live deployment schedule from API (matches Vercel env even when RSC props were cached). */
  const [apiSchedule, setApiSchedule] = useState(undefined);

  const stayOnDashboard = searchParams.get("stay") === "1";

  useEffect(() => {
    fetch("/api/program-schedule", { cache: "no-store" })
      .then(async (r) => {
        if (!r.ok) return null;
        return r.json();
      })
      .then((data) => {
        if (data) setApiSchedule(data);
      })
      .catch(() => {});
  }, []);
  ...
  const now = new Date();
  const programCtx =
    apiSchedule?.programCtx ??
    initialProgramCtx ??
    getProgramMissionContext(now);
  const missionCap =
    apiSchedule?.missionCap !== undefined
      ? apiSchedule.missionCap
      : initialMissionCap !== undefined
        ? initialMissionCap
        : skipMaxMissionDayCap(now)
          ? null
          : getMaxMissionDayCap();
  const catchUpRange =
    apiSchedule?.catchUpRange !== undefined
      ? apiSchedule.catchUpRange
      : initialCatchUpRange !== undefined
        ? initialCatchUpRange
        : getManualMissionCatchUpRange(now);

  const completedNums = completedDays.map((d) => d.day);
  const firstIncompleteCatchUp = getFirstIncompleteInRange(
    completedNums,
    catchUpRange
  );
  ...
  const nextQuizDay = programCtx.useLegacyProgression
    ...
    : firstIncompleteCatchUp != null
      ? firstIncompleteCatchUp
      : Math.min(40, programCtx.officialDay != null ? programCtx.officialDay : 1);

  useEffect(() => {
    if (loading || !studentId || stayOnDashboard) return;
    if (!programCtx.useLegacyProgression && programCtx.phase === "live") {
      if (catchUpRange) {
        const nums = completedDays.map((d) => d.day);
        if (allCompleteInRange(nums, catchUpRange)) return;
      } else if (programCtx.officialDay != null) {
        const doneToday = completedDays.some((d) => d.day === programCtx.officialDay);
        if (doneToday) return;
      }
    }
    router.replace(
  ...
  const readyDay =
    firstIncompleteCatchUp != null
      ? firstIncompleteCatchUp
      : !catchUpRange ||
          !allCompleteInRange(completedNums, catchUpRange)
        ? nextQuizDay
        : null;
  ...
  const startMission = () => {
    if (!studentId) return;
    if (!programCtx.useLegacyProgression && cohortMissionCompleted) {
      if (catchUpRange) {
        router.push(
          `/review?day=${catchUpRange.max}&student_id=${encodeURIComponent(studentId)}`
        );
        return;
      }
      if (programCtx.officialDay != null) {
        router.push(
          `/review?day=${programCtx.officialDay}&student_id=${encodeURIComponent(studentId)}`
        );
        return;
      }
    }
